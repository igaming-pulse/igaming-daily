...
依本檔「內容規格」章節收集。**v6.0 起改為三段式，順序不可對調**：

**第一段｜廣蒐候選（不查證）**
只收「標題＋原文 URL＋發布時間＋所屬分類」，**完全不做交叉查證、不抓內文、不抓 og:image**。
發布時間要記**含年份**的 `YYYY-MM-DD`（列表頁上的日期、網址裡的 `/2026/09/23/` 皆可）；
只看得到「Sep 22」這種沒年份的，先標 `年份待確認`，第三段再補（見「📅 主來源日期規則」）。
來源三塊，缺一不可：
1. **每日核心必掃 8 個**（見「📌 每日核心必掃清單」）—— 一定要掃，不受輪掃批次影響
2. **每日輪掃 5 個**（見「🔁 每日輪掃制」）—— 先跑 `python3 <REPO>/scripts/rotate_sources.py`
3. **WebSearch 廣撒**補長尾
...
**第二段｜打分排序（不花 credit）**
依「🧮 排序與取捨」章節對每則候選打分，套用三個硬上限與平手規則，取前 18（硬上限 22）。
...
確認該則有 1 個主來源、發布日期含年份且在窗內，**過不了就直接剔除、換下一名遞補**，不必再做後面的查證。
...
| # | 分類 | 則數 | 內容 | 格式 |
|---|------|------|------|------|
| 1 | 🎰 Game Provider 新遊戲（Slot） | **≤8** | 權重高的新 slot（強勢＋新興品牌）。**只收電子老虎機**，非 slot 放 cat2 | 三段式 |
| 2 | 🕹️ 非 Slot 新內容 | **3–5** | 非 slot 的新遊戲／新內容。**優先序**：① 小遊戲（Crash／Mines／Plinko）② Live Game ③ Poker／德州 ④ 地方棋牌 ⑤ 其他 | 三段式 |
| 3 | 🤝 動態：主流 GP／平台 | **≤7** | 大品牌或關鍵大事：合作、併購、互相提告、運營活動與成效 | 動態格式 |
| 4 | 🇵🇭 動態：菲律賓 GP／平台 | **商業新聞 ≤6**，PAGCOR／官方監理來源**另計不佔名額** | 菲律賓廠商／平台大事 ＋ 政府／在地新聞 | 動態格式 |
| 5 | 📊 市場數據 & 趨勢 | **2–5** | 市場趨勢優先（風靡玩法、影響產業的賽事／展會之實際影響） | 市場格式 |

**各區數字是上限，不是保證要收滿。** 真正的約束是下面這條：

> ### 🎯 總量：目標 18 則、**硬上限 22 則**
>
> 為什麼有硬上限：每則入選都要花 1 次 Firecrawl（抓內文＋og:image）。
> `列表頁 13 ＋ 入選 22 = 35 credits／天 × 30 天 = 1,050`，月額度 1,125，餘裕 6.7%。
> **超過 22 則就會吃掉補跑的緩衝**，這是預算的物理天花板，不是編輯偏好。

比例不必硬湊，**寧缺毋濫**：某類沒有夠份量的新聞就少收或省略，嚴禁灌水。
候選足夠時依「排序與取捨」章節的分數取前 18；候選不足就有幾則收幾則，**下限 13 則**（低於此在 Telegram 發出偏少警示）。
...
- **D**：與近 3 天已收同議題、且無新增實質資訊 `−4`（觸發後幾乎必定出局）
...
先依各區上限取足（cat1 ≤8、cat2 3–5、cat3 ≤7、cat4 ≤6＋官方、cat5 2–5），
**若某區在前 18 名內一則都沒有，從全池該區最高分的補 1–2 則進來**，避免整段消失 ——
即使分數低於其他落選項。cat5 特別容易觸發這條。
...
- **整場硬上限**：`firecrawl ≤ 35 次`、`WebSearch ≤ 60 次`。
  ...
  拆法（v6.0）：**核心必掃 8 ＋ 輪掃 5 ＋ 入選文章 ≤22（每則原則 1 次）**。
...
### 📌 每日核心必掃清單（v6.0 新增，硬規則）

**不論輪掃批次算到哪幾個，下面 8 個列表頁每天都要掃。** 這是低重疊率的主要解方 ——
舊版所有來源都在輪替池裡排隊，連每天都有稿的核心媒體平均也要 21 天才被深挖一次。

| # | 名稱 | URL | 主要補 |
|---|------|-----|--------|
| 1 | SlotBeats | https://slotbeats.com/ | 🎰 Slot 新遊戲 |
| 2 | CasinoBeats | https://casinobeats.com | 🎰 Slot ＋ 🕹️ 非 Slot ＋ 🤝 |
| 3 | Yogonet International | https://www.yogonet.com/international/ | 🤝 ＋ 📊 |
| 4 | SBC News | https://sbcnews.co.uk | 🤝 |
| 5 | iGaming Business | https://igamingbusiness.com/news/ | 🤝 ＋ 📊 |
| 6 | Gambling Insider | https://gamblinginsider.com/news/ | 📊 市場數據、M&A、財報 |
...
- 已知弱點：這 8 個沒有專門做 🕹️ 非 Slot（真人娛樂場／Crash／Bingo／撲克）的媒體，
  cat2 靠 CasinoBeats／SBC／iGB 順帶覆蓋 —— **已知並接受**
...
（半夜全新無記憶的 session 也算得出同一批），約 **34 天**把 **168 個新聞型來源**覆蓋一輪。

1. **取今日批次**
   ```bash
   python3 <REPO>/scripts/rotate_sources.py
   ```
   輸出今天輪值的 **5 個**來源，每行 `名稱<TAB>分類<TAB>URL`。
...
4. **預算**：**5 次 firecrawl／天**（＝5 credits）。
   v6.0 由 8 降為 5 —— 額度挪去支應「每日核心必掃 8 個」。覆蓋週期 21 天 → **約 34 天**，
   是**已知且接受的代價**：長尾來源本來就不是每天有新聞，核心 8 個的穩定覆蓋價值更高。

### 🔦 加碼規則（v6.0 改寫：看候選池，不看成稿則數）

**觸發點改在第一段結束時**，而不是等成稿才發現不夠：

- **候選池 < 25 則** → 立刻加碼廣蒐：針對不在今日輪掃批次內的 ⭐ 優先品牌官網 ＋「產業媒體」分類多挖
- **候選池 ≥ 25 則** → 不加碼，直接進第二段排序

**為什麼改**：舊版是「查證完才發現總數 < 15，回頭補」—— 那時預算已經花在查證上，來不及。
新版在還沒開始查證時就知道池子有多大，加碼花的是**發現預算**，成本低得多。
這也把「今天到底淡不淡」從事後推測變成**事前可見的事實**。

加碼同樣不另外開額度，一律在整場 35 次總額內。池子補到 25 則即停；
額度用完仍 < 25 就**老實少收**（寧缺毋濫），不可灌水，並在 Telegram 品質備註誠實說明。
